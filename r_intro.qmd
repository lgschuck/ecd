...
Para criação de variáveis do tipo texto, devem ser usadas aspas, simples ou duplas. Aqui o **objeto2** foi criado com uso de aspas ao início e ao final de **texto** para que o R trate o valor como *character*. Caso Não sejam colocadas as aspas, o R entenderá que texto é o nome de um objeto.
...
O vetor objeto3 foi criado com 5 posições, armazenando inicialmente os valores de 1 a 5. Após o vetor recebeu atualização dos valores para 11, 12, 13,14 e 15.

Os valores dos vetores podem ser acessados através de seu índice em combinação com o operador de extração **\[**. Em R os índices iniciam em 1 (em muitas linguagens de programação o primeiro índice é 0).
...
Note que sem o operador de atribuição o valor da posição 4 do objeto3 não é atualizada, apenas exibida no console. Para atualizar seu valor usamos devemos usar a atribuição.
...
A função `is.vector`, por sua vez, testa se um objeto é um vetor e retorna um valor lógico como resposta do teste, `TRUE` ou `FALSE`.
